-- ============================================================================
-- MWRD MARKETPLACE - COMPLETE DATABASE SETUP
...
-- Auto-calculate final price on quote
...
CREATE POLICY "Suppliers can delete own pending products" ON products FOR DELETE USING (auth.uid() = supplier_id AND status = 'PENDING');
...
CREATE POLICY "Clients can update own open RFQs" ON rfqs FOR UPDATE USING (auth.uid() = client_id AND status = 'OPEN');
CREATE POLICY "Suppliers can view relevant RFQs" ON rfqs FOR SELECT USING (
  get_user_role() = 'SUPPLIER' AND EXISTS (
    SELECT 1 FROM rfq_items ri JOIN products p ON p.id = ri.product_id
    WHERE ri.rfq_id = rfqs.id AND p.supplier_id = auth.uid()
  )
);
...
-- RFQ ITEMS POLICIES
CREATE POLICY "Clients can view own RFQ items" ON rfq_items FOR SELECT USING (
  EXISTS (SELECT 1 FROM rfqs WHERE rfqs.id = rfq_items.rfq_id AND rfqs.client_id = auth.uid())
);
CREATE POLICY "Clients can create RFQ items" ON rfq_items FOR INSERT WITH CHECK (
  EXISTS (SELECT 1 FROM rfqs WHERE rfqs.id = rfq_items.rfq_id AND rfqs.client_id = auth.uid())
);
CREATE POLICY "Suppliers can view relevant RFQ items" ON rfq_items FOR SELECT USING (
  EXISTS (SELECT 1 FROM products p WHERE p.id = rfq_items.product_id AND p.supplier_id = auth.uid())
);
...
CREATE POLICY "Suppliers can create quotes" ON quotes FOR INSERT WITH CHECK (
  auth.uid() = supplier_id AND get_user_role() = 'SUPPLIER' AND EXISTS (
    SELECT 1 FROM rfq_items ri JOIN products p ON p.id = ri.product_id
    WHERE ri.rfq_id = quotes.rfq_id AND p.supplier_id = auth.uid()
  )
);
CREATE POLICY "Suppliers can update pending quotes" ON quotes FOR UPDATE USING (auth.uid() = supplier_id AND status = 'PENDING_ADMIN');
CREATE POLICY "Clients can view quotes for their RFQs" ON quotes FOR SELECT USING (
  status IN ('SENT_TO_CLIENT', 'ACCEPTED', 'REJECTED') AND EXISTS (
    SELECT 1 FROM rfqs WHERE rfqs.id = quotes.rfq_id AND rfqs.client_id = auth.uid()
  )
);
CREATE POLICY "Clients can accept/reject quotes" ON quotes FOR UPDATE USING (
  status = 'SENT_TO_CLIENT' AND EXISTS (
    SELECT 1 FROM rfqs WHERE rfqs.id = quotes.rfq_id AND rfqs.client_id = auth.uid()
  )
);
...
CREATE POLICY "Admins can delete margin settings" ON margin_settings FOR DELETE USING (get_user_role() = 'ADMIN');
...
BEGIN
  ...
  VALUES (
    ...
    FALSE,
    CASE WHEN COALESCE(NEW.raw_user_meta_data->>'role', 'CLIENT') = 'SUPPLIER' THEN 'PENDING'::user_status ELSE 'ACTIVE'::user_status END,
